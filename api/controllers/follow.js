...
function saveFollow(req, res){
	...
	follow.save((err, followStored)=>{
		if(err)return res.status(500).send({ menssage: 'Error al guardar el seguimiento'});
...
function getFollowingUsers( req, res ){
	...
	.paginate(page, itemsPerPage,(err,follows,total)=>{
		...
		if(!follows){
			return res.status(404).send({ menssage: 'No estas siguiendo a ningun usuario'});
		}
...
function getFollowedUsers( req, res){
	...
	.paginate(page, itemsPerPage,(err,follows,total)=>{
		...
		if(!follows){
			return res.status(404).send({ menssage: 'No te sigue ningun usuario'});
		}
...
function getMyFollows( req, res ){
	...
	find.populate('user followed').exec( (err, follows)=>{
		...
		if(!follows){
			return res.status(404).send({ menssage: 'No sigues a ningun usuario'});
		}
